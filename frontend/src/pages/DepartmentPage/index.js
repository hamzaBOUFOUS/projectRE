...
export default function ListDepartments(props) {
    ...
    const handleGetDepartments = useCallback(
        (nPage, nFilter) => {
            dispatch(getListDepartments(nPage, nFilter, 10));
        },
        [dispatch]
    );
    ...
    useEffect(() => {
        handleGetDepartments(page, filter);
    }, [handleGetDepartments, filter]);
    return (
        <>
                                        ...
                                        <Loader status={status}>
                                                ...
                                                <TableBody>
                                                    ...
                                                    {departments.map((department, idx) => (
                                                        <TableRow className={cx({ [classes.coloredRow]: idx % 2 === 0 })}>
                                                            ...
                                                            <TableCell>{department.nomDepartment}</TableCell>
                                                            <TableCell>{department.dateCreation}</TableCell>
                                                            <TableCell className={classes.actions}>
                                                                <IconButton
                                                                    aria-haspopup="true"
                                                                    onClick={handleOpenAddEdit(department)}
                                                                    color="secondary"
                                                                >
                                                                    <Edit fontSize="small" />
                                                                </IconButton>
                                            ...
                                            <CustomPagination
                                                ...
                                                onPageChange={(_, newPage) => {
                                                    handleGetDepartments(newPage - 1, filter);
                                                }}
                                            />
                                        </Loader>
                                    </TableContainer>
