...
import { deleteDocumentDemande } from "../../../stores/reducers/documentDemande/actions";
import {
  Dialog,
  Button,
  DialogActions,
...
export default ({ open, handleClose, selected }) => {
  ...
  const deleteDocumentDemandeCallback = useCallback(() => {
    dispatch(
      deleteDocumentDemande(selected.id, handleClose)
    );
  }, [dispatch, selected, handleClose]);
  
  function submitForm() {
    deleteDocumentDemandeCallback();
  }
  return (
    <Dialog
      maxWidth="xs"
